...
class Superimposition:
    ...
    def run(self):
        ...
        log_subsection('Extract docking quality metrics')
        self._extract_docking_metrics()
        df_super = log_subsection('Superimpose docked structures')
        self._superimposition()
        log_subsection('Calculate protein and ligand RMSDs')
    ...
    def _superimposition(self):      
        prepare_files_for_superimposition(df_vina= Path(self.input_dir)/'vina.pkl', df_chai= Path(self.input_dir)/'chai.pkl', ligand_name=self.ligand_name, output_dir = Path(self.input_dir) / 'preparedfiles_for_superimposition')
        df = pd.DataFrame()
        df_sup = df << (SuperimposeStructures('vina_files_for_superimposition',  'chai_files_for_superimposition',  output_dir = self.output_dir, name1='vina', name2='chai', num_threads = self.num_threads) 
                ...
                >> Save(f'{self.output_dir}/superimposedstructures.pkl'))
        return df_sup
    
    def _proteinRMSD(self, df):  
        proteinRMSD_dir = Path(self.output_dir) / 'proteinRMSD'
        proteinRMSD_dir.mkdir(exist_ok=True, parents=True) 
        df_proteinRMSD = df << (ProteinRMSD('Entry', input_dir = self.output_dir, output_dir = proteinRMSD_dir, visualize_heatmaps = True)  >> Save(Path(self.output_dir)/'proteinRMSD.pkl'))
