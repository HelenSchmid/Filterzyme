...
#SBATCH --time=1-00:00:00               # 5 days time limit
...
# Setup local scratch
SCRATCH_DIR="/scratch/ariane/PDE_2H/"
mkdir -p "$SCRATCH_DIR"
...
cp PDE_data_formatted.csv $SCRATCH_DIR
cp run_PDE_2H.py $SCRATCH_DIR
cd $SCRATCH_DIR

# Run filrerzyme
echo "Starting filterzyme ..."
python3 run_PDE_2H.py

# Copy results back to persistent storage
echo "Copying results back to home directory..."
#mkdir -p $HOME/results/job_$SLURM_JOB_ID
cp -r $SCRATCH_DIR/filterzyme_output/ /mnt/labs/data/mora/code/Filterzyme/benchmarking/PDE_2H/
